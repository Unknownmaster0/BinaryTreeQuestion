...
vector<int> zigZag(Node *root)
{
    ...
    while (!q.empty())
    {
        // level wise traversal krna hai.
        // Now, finding the elements in the each level.
        int size = q.size();
        vector<int> temp(size);
        cout << "the size of temp vector -> " << size << endl;

        // traversing all the elements of the particular level.
        ...
        */
        ...
        cout << endl
             << "Printing the temp vector" << endl;
        printVector(temp);
        cout << endl;
        // left to right ka direction change kr diye hai.
        LTR = !LTR;

        // pushing the elements of the temp in the ans.
        for (auto i : temp)
            ans.push_back(i);
    }
    return ans;
